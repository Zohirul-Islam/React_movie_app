import React from 'react'
import { useSelector } from 'react-redux';
import { Link } from 'react-router-dom';

const Card = ({ data, index, trending, media_type }) => {
  const imgUrl = useSelector((state) => state.movieData.imageUrl);
  const mediaType = data.media_type ?? media_type;

  return (
    <Link 
      to={`/${mediaType}/${data.id}`} 
      className="w-full max-w-[230px] min-w-[235px] h-80 overflow-hidden block hover:scale-105 transition-all rounded-lg relative"
    >
      <img 
        src={data.poster_path ? imgUrl + data.poster_path : "/no-image.png"} 
        alt={data?.title || data?.name || "Poster"} 
        className="w-full h-full object-cover" 
      />

      {trending && (
        <div className="absolute top-4">
          <p className="py-1 px-4 bg-black/60 backdrop-blur-3xl rounded-r-full overflow-hidden">
            #{index + 1} {trending}
          </p>
        </div>
      )}

      <div className="bottom-0 h-16 bg-black/60 p-2 backdrop-blur-3xl w-full absolute">
        <h1 className="text-ellipsis line-clamp-1 text-lg font-semibold">
          {data?.title || data?.name}
        </h1>
        <div className="text-sm text-neutral-400 flex justify-between items-center">
          <p>{data.release_date || data.first_air_date || "N/A"}</p>
          <p className="bg-black rounded-full p-1 text-xs text-white">
            Rating: {data.vote_average ? Number(data.vote_average).toFixed(1) : "N/A"}
          </p>
        </div>
      </div>
    </Link>
  );
}

export default Card;
